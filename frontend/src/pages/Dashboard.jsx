...
const Dashboard = () => {
  ...
  const areaData = [
    ...
    { day: "Sun", success: stats.success || 8, failed: stats.failed || 1, pending: stats.pending || 3 },
  ];
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Monitor your webhook delivery in real-time</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
        ...
        <div className="xl:col-span-2 glass-card p-6">
          ...
          <p className="text-gray-500 text-xs mb-5">Last 7 days webhook activity</p>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-gray-500 text-xs uppercase tracking-wider border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
              ...
              <tbody className="divide-y" style={{ '--tw-divide-opacity': 1, borderColor: "rgba(255,255,255,0.04)" }}>
                {recent.map((w) => (
                  <tr key={w._id} className="table-row-hover transition-colors">
                    <td className="py-3 pr-4 text-gray-300 font-mono text-xs max-w-[200px] truncate">{w.url}</td>
                    <td className="py-3 pr-4"><StatusBadge status={w.status} /></td>
                    <td className="py-3 pr-4 text-gray-400">{w.retryCount}</td>
                    <td className="py-3 text-gray-500 text-xs">{new Date(w.createdAt).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
